...
export function OnboardingForm() {
  ...
  const adSpendParam = Number(searchParams.get("adSpend"))
  const adSpend = plan?.hasAdSpend && adSpendParam > 0 ? adSpendParam : undefined
  ...
  const [form, setForm] = useState<IntakeSubmission>({
    ...
    voiceTone: "",
    targetAudience: "",
    contact: { phone: "", address: "", website: "", socialHandles: "" },
  ...
  return (
    <div>
      ...
      <p className="mt-2 text-sm text-muted-foreground">
        {plan ? (
          <>
            You subscribed to <span className="text-foreground font-medium">{plan.name}</span>
            {plan.hasAdSpend && adSpend ? (
              <>
                {" "}
                with a <span className="text-foreground font-medium">${adSpend.toLocaleString()}/mo</span> ad budget
              </>
            ) : null}
